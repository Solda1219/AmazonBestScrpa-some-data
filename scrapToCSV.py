...
class BestSellerScraper():
    ...
    def select_fromJson(self):
        ...
        f= open('posts.json', encoding='utf-8-sig')
    ...
    def select_scrap(self, base_url):
        ...
        if driver== 0:
            print('Something went wrong! Maybe the Chrome has updated, so the Chrome driver version is not matched the Chrome')
            print('Please update driver version or just install chrome of version 86!')
            return 0
        driver.get(base_url)
        time.sleep(5)
    ...
    def urlscrape(self, unitUrl, categoryID, lock):
        ...
        if driver== 0:
            ...
            self.threadFlag+= 1
            lock.release()
            print('Warning! You have ran too many threads.')
            print('Maybe you will lose many data, please set modifyNum to be short. standard: 5')
            return 0
        driver.get(unitUrl)
        ...
        if len(itemDoms)>1 and itemDoms[0].find('div', attrs= {'class': 'p13n-sc-truncate-desktop-type2 p13n-sc-truncated'})== None:
            time.sleep(TimeForDes)
            soup = BeautifulSoup(driver.page_source, 'html.parser')
            itemDoms= soup.find_all('li', attrs={'class': 'zg-item-immersion'})
        items=[]
    ...
    def scrape(self):
        ...
        if urls== 0:
            print("Couldn't get urls because of Chrome driver problem!")
            return 0
